            ...
            <?php foreach ($records as $record): ?>
                <li>
                    <?php if (!empty($record->syncData['direction'])
                        && $record->syncData['direction'] == 'push'
                    ): ?>
                        <?php _e('Pushed', 'xenforo-api-consumer'); ?>
                    <?php elseif (!empty($record->syncData['direction'])
                        && in_array($record->syncData['direction'], array('pull', 'subscription'), true)
                    ): ?>
                    ...
                    <ul style="margin-left: 20px;">
                        ...
                        <li><a href="<?php echo admin_url(sprintf('tools.php?action=xfac_tools_sync&type=%s&sync_id=%d',
                                $record->provider_content_type, $record->sync_id)) ?>"><?php _e('Manual Sync',
                                    'xenforo-api-consumer'); ?></a></li>
